...
void setupFonts(void) {
  ...
  for(int i=0; i<tableLen; i++){
    ...
    if (existsFile(fontPath)) {
      font = pio.Fonts->AddFontFromFileTTF(fontPath, point2px(14.5)
          , config
          , pio.Fonts->GetGlyphRangesJapanese());
  ...
  if (font == NULL) {
    ...
    pio.Fonts->AddFontDefault(NULL);
  }
  // Merge IconFont
  config->MergeMode = true;
  pio.Fonts->AddFontFromFileTTF(IconFontPath, point2px(11), config , ranges_icon_fonts);
}
